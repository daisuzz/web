...
Rust向けの[[async-runtime]]。Rust言語自体は非同期ランタイムを標準搭載しておらず、`async fn`は「実行するとFutureという状態機械の値を返すだけ」の構文糖衣で、それを実際にポーリングして進める仕組みは別途用意する必要がある。複数の実装が並立しうる設計だが、実質的にTokioがデファクトスタンダードになっている([[deno]]の内部実装も実際にTokioを採用している)。
