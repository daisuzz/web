...
2. frontmatterに `created`（今日の日付、`"2026-08-18"`形式）を書く。既存ノートを更新する場合は `updated` を今日の日付に更新する。（`src/content/loadNotes.ts` はfrontmatterの値をそのまま使うだけで、git履歴やファイルのmtimeからは推測しない）。`/notes/`一覧の並び順は`created`の実際の日時で決まるため、同じ日に複数ノートを作成して順序を厳密にしたい場合は`created: "2026-08-18T14:30:00+09:00"`のように時刻まで書ける。時刻を省略した場合はその日の0時(UTC)扱いになる。画面表示は時刻の有無によらず常に日付のみ
