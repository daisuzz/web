...
nginx/MySQLなどの設定ファイルをサーバーをまたいでチームで共有・追跡する具体的な手順とスクリプトは[[isucon-etc-config-management]]にまとめてある。
...
ここまで終えたら、実際にベンチマークを回しながら次の「基本サイクル」に入る。Go実装での具体的なコマンド・コード断片は[[isucon-go-runbook]]にまとめてある。
