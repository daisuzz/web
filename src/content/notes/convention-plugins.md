...
複数プロジェクトに共通するプラグイン適用・設定・デフォルト値をひとまとめにした、再利用可能なビルドロジック。大きい・マルチプロジェクトのビルドで、各サブプロジェクトの`build.gradle.kts`に同じ`plugins {}`ブロックや設定を毎回コピペする代わりに、1つの独自プラグインとして書いて`apply`する。
