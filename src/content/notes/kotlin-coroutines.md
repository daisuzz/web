---
created: "2026-08-18"
updated: "2026-09-16T19:12:00+09:00"
---
# Kotlin Coroutines
...
- **Java Virtual Threads (Project Loom)**: JVMが管理する軽量スレッドで、フル継続(スタックそのもの)を保持する。既存の同期的・ブロッキングなコードをほぼ書き換えずに使える利点があるが、構造化並行性は標準では強制されず、`StructuredTaskScope`を使う側が意識的に採用する必要がある（[[structured-concurrency]]参照）。I/Oバウンドな超高負荷(100万リクエスト規模)ではKotlin coroutinesがVirtual Threadsをやや上回るというベンチマーク報告もある。
